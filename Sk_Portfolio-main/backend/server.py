...
# MongoDB connection (optional for now — app still runs without it;
# only /api/status and /api/contact persistence require it to be set)
...
if mongo_url and db_name:
    ...
    logger.warning(
        "MONGO_URL / DB_NAME not set — running without a database. "
        "Projects endpoint still works; contact messages will be emailed "
        "(if RESEND_API_KEY is set) but not persisted until MongoDB is configured."
    )
...
PROJECTS_DATA: List[Project] = [
    ...
    Project(
        id="aureus",
        name="Aureus",
        description="TypeScript-powered product experiment exploring elegant interface primitives and interaction patterns — a design-engineering playground for polished, opinionated components.",
        language="TypeScript",
    ...
    Project(
        ...
        description="Collaborative AI-powered learning platform (HarshilxAI/Cognify_Learning) — adaptive study modules, contextual quizzes, and knowledge graphs that evolve with the learner.",
        language="JavaScript",
...
async def submit_contact(payload: ContactCreate):
    # Attempt email send (works independently of the database)
    ...
    if db is not None:
        ...
        logger.info(
            "Contact message from %s not persisted (no database configured): %s",
            payload.email,
            payload.subject,
        )
